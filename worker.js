    ...
    if (isOwner) {
        ...
        if (reply_to_message) {
            ...
            if (target) {
                // 将固定目标切换为该用户，并转发回复
                const mins = 10;
                await kv.put(`reply_target:${owner}`, target);
                await kv.put(`reply_target_expire:${owner}`, (Date.now() + mins * 60000).toString());
                await tg('copyMessage', { chat_id: parseInt(target), from_chat_id: chatId, message_id: msg.message_id });
                // 发送确认信息
                let targetName = '';
                try {
                    const info = await tg('getChat', { chat_id: parseInt(target) });
                    const u = info.result || {};
                    targetName = u.username ? `@${u.username}` : (u.first_name || '用户');
                } catch { targetName = '未知用户'; }
                await tg('sendMessage', {
                    ...
                    reply_markup: {
                        inline_keyboard: [[
                            ...
                            { text: '🔗 取消固定回复', callback_data: `unlock_${target}` }
                        ]]
                    }
                });
            }
        ...
        if (replyTarget && (text || caption) && !text?.startsWith('/')) {
            ...
            let targetName = '';
            try {
                const info = await tg('getChat', { chat_id: parseInt(replyTarget) });
                const u = info.result || {};
                targetName = u.username ? `@${u.username}` : (u.first_name || '用户');
            } catch { targetName = '未知用户'; }
            await tg('sendMessage', {
                chat_id: chatId,
                text: `📤 已发送给 ${targetName} (${replyTarget})`,
                reply_markup: {
                    inline_keyboard: [[
                        { text: '🚫 封禁该用户', callback_data: `ban_${replyTarget}` },
                        { text: '🔗 取消固定回复', callback_data: `unlock_${replyTarget}` }
                    ]]
                }
            });
            return;
